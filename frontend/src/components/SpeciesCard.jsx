        ...
        >
            <div className="relative aspect-square overflow-hidden bg-white">
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
                    <span className="bg-white text-primary px-4 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                        View Data <span className="material-symbols-outlined text-sm">open_in_new</span>
                    </span>
                </div>
                ...
                {species.iucnStatus && (
                    <div className="absolute top-3 right-3 z-10">
                        <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border shadow-sm backdrop-blur-md ${getStatusColor(species.iucnStatus)}`}>
                            {species.iucnStatus}
                        </span>
                    </div>
                )}
            ...
            <div className="p-5 flex flex-col bg-white dark:bg-[#1a2620]">
                {/* Local name if available, otherwise common name */}
                <h4 className="text-xl font-extrabold text-[#121614] dark:text-white mb-1 line-clamp-1 group-hover:text-primary transition-colors">
                    {species.localName || species.commonName}
                </h4>

                {/* Fallback to common name if localname was used above, otherwise scientific name */}
                {species.localName ? (
                    <p className="text-xs font-bold text-[#6a8174] uppercase tracking-wider mb-1 line-clamp-1">{species.commonName}</p>
                ) : null}

                <p className="text-sm italic text-[#6a8174] line-clamp-1">{species.scientificName}</p>
            </div>
        </Link>
    );
};
